...
**As of 2026-06-30, all 9 non-calendar strategies use a uniform fixed-SL + RR-TP policy (user decision).** When a strategy's config block sets `sl_points` (a FIXED price-point stop = the inputted SL), `RiskProcessor.calculate_stops()` takes a generic branch FIRST: `SL = entry ∓ sl_points`, `TP = entry ± sl_points × rr` (config `rr`, default 2.0 → clean 1:1/1:2/1:3), and it sets BOTH `preserve_structural_sl` + `preserve_structural_tp` so the execution-layer BudgetSL/BudgetTP do NOT rewrite the geometry; the liquidity-nudge is also skipped to keep R:R exact. Converted (all on XAUUSD): `kalman_regime`, `vwap`, `momentum`, `sbr`, `asia_range_fade`, `smc_ob`, `fibonacci_retracement`, `stoch_pullback`, `squeeze_breakout`. A block WITHOUT `sl_points` falls through to its legacy per-strategy ATR branch (backward compatible). The 4 calendar/drift strategies (`london_breakout`, `monday_drift`, `index_overnight`, `wednesday_drift`) are EXCLUDED — no-TP by design. ⚠️ This changes kalman's live SL from budget-governed (`risk_per_trade_usd` was the DD lever) to the fixed `sl_points` (now preserved), and replaces stoch's structural SL — both validated edges overridden by user choice. Seed `sl_points: 33.0` is a PLACEHOLDER in every block — tune per strategy. Position size is still computed off the fixed SL (so per-trade $ risk floors at min-lot, same as squeeze).

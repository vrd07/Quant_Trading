...
→ StrategyManager (8 strategies emit Signals)
→ StrategyAllowlist (default-deny: only SOLO_ALLOWED names pass)
...
**As of 2026-08-13, `ConfluenceGate` is DELETED and replaced by `StrategyAllowlist` (`src/strategies/strategy_allowlist.py`).** The combo policy from `combine_startegy.md` (COMBO A/B/C, `combo_sniper` at 1.5×, the exhaustion modifier) never fired a single combo in live trading — including after the window was widened 25 → 120 minutes — so the six confluence-only legs and the machinery consuming them were removed together. What remains is default-deny: `SOLO_ALLOWED` lists the eight strategies permitted to execute, `KILL_LIST` holds all 14 deleted names so a stale config cannot resurrect one. There is no enable flag by design — a switch that turns a safety net off is not a safety net. Verified behaviour-identical: the XAUUSD 15m ensemble backtest is byte-identical before vs after (1,711 trades, PF 1.38).

**As of 2026-06-10, the six kill-list strategies were DELETED from the codebase** (no backtested edge — disabled live for weeks). Removed: `breakout`, `mean_reversion`, `supply_demand`, `descending_channel_breakout`, `mini_medallion`, `continuation_breakout` — files, tests, config blocks, and `STRATEGY_WEIGHTS` keys are gone. Their names remain in `StrategyAllowlist.KILL_LIST` as a defensive net so a stale config can't crash the registry lookup. Git history preserves the deleted code.

**As of 2026-08-13, eight more were DELETED in a second round.** Six (`vwap`, `momentum`, `sbr`, `asia_range_fade`, `smc_ob`, `fibonacci_retracement`) existed only as `ConfluenceGate` combo legs and could not reach the risk engine under any config in the repo; two (`wavelet_cycle`, `ema200_nasdaq`) were already `enabled: false` with published refutations — see the REJECTED notes below, which outlive the code. Files, tests, config blocks, session whitelists, `STRATEGY_WEIGHTS` keys, the `symbols.NAS100` block and the `symbol_reconciler` mapping are all gone; the 14 names live on in `KILL_LIST`.
...
| # | Strategy | File | Gate status | Summary |
|---|----------|------|------------------------|---------|
...
**Researched and REJECTED 2026-08-09: H4 double-FVG tap** (`scripts/research_double_fvg.py`, report `reports/double_fvg_research.md`). User-authored rule shipped in the chart EA `mt5_bridge/GoldHTF_AutoOpt_EA.mq5` v3.00: two same-direction H4 fair-value gaps within 12 bars arm the FIRST-formed gap; a closed 5m candle tapping 50–60% into it (closing in the gap's direction) enters with a structural stop and RR 2.0. XAUUSD 2022-01→2026-08, strict fills, causal replica of the EA's per-H4-bar scan cadence. **Spec cell: PF 1.09 (ladder exit) / 1.10 (fixed TP), 34 trades/yr, 2 of 6 backtest.md gates passed** (fails G1 dayWR 59.6%, G2 worst day −2.04R, G3 PF, G6 frequency). ⚠️ **The decisive test is the matched random-entry control** — same trade count, same side mix, stop widths resampled from the real trades, random entry bars: the rule beats only **58% of 200 random draws** (null median PF 1.02, p95 1.64). ⚠️ **Ablation kills the premise directly:** arming the FIRST gap (PF 1.09) vs the SECOND (0.98) vs dropping the pair requirement entirely (1.04) all sit inside the null's spread — neither the "double" nor the "first" is load-bearing. Sweep is jagged (only 10/27 cells IS-positive) and `tap_min=75` beats the specified 50, i.e. the named band isn't where anything lives. No variant is positive every calendar year. **UPDATE 2026-08-09: user chose to enable it anyway** (`InpUseDoubleFVG: true`), pending their own MT5 Strategy Tester run — the no-edge verdict above stands and is unchanged; revert = one input. Do not re-research this family with a wider grid — that is the `project_rsi_reversal_m1` failure mode. ⚠️ Separately measured and worth keeping: the v3.00 **profit ladder is a risk-profile tool, not an edge tool** — vs a plain fixed TP it lifts win rate 39%→55% and halves max DD (−16.3%→−7.2%) at essentially unchanged PF (1.10→1.09), and it is WORSE under $5k risk enforcement (PF 1.14→0.84). It trades runners for frequency of small wins, which is the same mechanism that killed the liquidity TP overlay.

**Researched and REJECTED — `ema200_nasdaq` (NY-open EMA200 anchor break), CODE DELETED 2026-08-13.** The strategy file, config blocks, `symbols.NAS100` block and `symbol_reconciler` mapping are gone (git history preserves them); the finding below is why, and it stands. **Added 2026-07-07; DISABLED in ALL live configs 2026-07-08** (`enabled: false` — negative expectancy in every framing; RR1:1 last-12mo −$13.6k at $150 risk, WR 43% vs 59% breakeven, 56% of losses gap past the anchor-wick stop). **v2 redesign REJECTED same day** (`scripts/research_ema200_v2.py`, 96-variant grid: DST-aware cash-open anchor / overnight-range + daily-EMA20 biases / pullback-limit entry / 1×ATR stop floor / forced EOD flat): every fix helped directionally — best cell (cash-open + overnight-range bias + chase + ATR floor + RR2) lifts PF 0.58→1.37 — but the year map is 1.06/2.13/1.10 = 2025-only regime beta; **0/96 clear the every-year gate. Pullback-limit entry is WORSE than the chase everywhere (limit-fill adverse selection). Do NOT re-research the NY-open anchor-break family.** Report: `reports/ema200_v2_research.md`. Original spec, NASDAQ-100 ONLY — CONFIGURABLE ticker (user-authored, `new_strategies.md` #2). Rule: ANCHOR = the 5m candle at 19:10 IST = **13:40 UTC fixed** (`anchor_hour_utc`/`anchor_minute_utc`); anchor close above EMA(200) → the FIRST later 5m close above the anchor close by 15:40 UTC (`entry_end_*`) enters BUY (mirror below for SELL); **strictly one entry/day**; **SL = anchor candle's opposite extreme; TP = `rr`(2.0)×stop-dist** (`preserve_structural_sl`). Research (`research_ema200_nas.py`, 2.5y NAS100 Dukascopy, strict): **PF 1.04 full; 2024 0.90 / 2025 1.41 / 2026 0.79** — one good year = the one-way bull leg (regime beta); raw DD −31.5%; EOD-close variant identical. **Production backtest (`run_backtest --timeframe 5m --slippage strict`): raw PF 0.58 −42.9%; --enforce-risk PF 0.19, halted −5.2%** (and the strict fill tables charge ZERO spread on indices — optimistic). **Symbol is configurable:** broker lists NASDAQ-100 under its own ticker — `runtime_setup.py` Step 1's broker-ticker rename now also rewrites ANY gated strategy's `allowed_symbols` via a `strategies:` override in `runtime_overrides.yaml` (generic; fixes prefix-gate misses like NAS100→USTEC). `symbols.NAS100` blocks (PLACEHOLDER spec — VERIFY vs broker) + `strategy_whitelist:[ema200_nasdaq]` in all 8 configs; `symbol_reconciler._STRATEGY_SYMBOLS` maps it. **Backtest `--timeframe 5m`** (native spec TF). Report: `reports/ema200_nasdaq_research.md`.

**Researched and REJECTED — `wavelet_cycle` (wavelet + spectral cycle engine), CODE DELETED 2026-08-13.** The strategy file and its config blocks are gone; `src/cycles/` is RETAINED (the measurement machinery and its documented traps are still worth having). The finding below stands. **Added 2026-08-09, XAUUSD ONLY (hard symbol gate in code) — SHIPPED `enabled: false` IN ALL 8 CONFIGS; research verdict = NO EDGE.** Causal wavelet+spectral cycle engine (`src/cycles/`): à-trous DWT trend → Blackman-Harris FFT / Burg MESA dominant-cycle estimate → Goertzel phase tracker + Ehlers roofing filter → ADX/Hurst/vol regime gate. Entries need THREE independent confirmations (structural: price vs delay-compensated trend; cyclic: Goertzel phase; regime); UNCERTAIN regime ⇒ flat. SL = 2.0×ATR (2.5× above the 70th vol pctile, 1.5× below the 30th), TP = `rr`×stop for TREND / the trend line for MEAN_REVERT, `preserve_structural_sl`. **Research (`scripts/research_wavelet_cycle.py`, walk-forward train 2022-01→2024-01 / validate →2024-07 / test →2025-01, strict fills, surrogate control): NO cell was positive on BOTH train and validate on either asset, so the frozen OOS slice was NEVER SPENT and stays clean.** XAUUSD train PF 0.94–1.25 (17–46 trades), validate 0.00–0.48 (4–10 trades). BTCUSD **0 trades in all 12 cells** — verified NOT a plumbing null: the regime detector classifies normally (TREND 795/UNCERTAIN 751/MEAN_REVERT 54 over 1,600 bars) but the cycle gate never opens (`low_prominence` 1,600/1,600). ⚠️ **Agrees with `reports/fourier_research.md`** rather than overturning it — that pass left open exactly one door ("spectral features as ONE input among several in a regime classifier"), and this spec is that door built properly; it still shows no edge. ⚠️ **DEPARTURE FROM SPEC: `min_prominence` 4.0, not 2.0.** At 2.0 the §8.1 hallucination gate FAILS outright — pink noise reads "tradeable" on 61% of bars, ARMA(2,1) 41%, vs a 30% ceiling; a pure random walk scored prominence ≥2.0 on 40/40 trials until the measure itself was rebuilt (scored against the spectrum's own power-law background, not the band mean; MESA order n//3→n/9 which was the biggest single lever). Measurement table lives in `src/cycles/cycle_tracker.py`'s docstring. ⚠️ **Measured group delay is much heavier than the spec's "~8–12 bars" guess** (that assumes a decimated DWT; the à-trous cascade is heavier): db4 L2 = **17.8 bars** (~4.5h on 15m gold), sym8 L3 = **48 bars** (~2 days on 1h BTC). The trend line is a slow structural reference, NOT a timing signal. ⚠️ On real 15m gold the engine calls a cycle tradeable on only **2.8% of bars**, and the median detected period is **44 bars against a 48-bar cap** — it usually locks onto the low-frequency end of its own search band, which is what a 1/f² spectrum looks like when you ask it for a cycle. **Registered in `STRATEGY_REGISTRY` + all 3 `STRATEGY_WEIGHTS` regimes at weight 0.00** (so flipping `enabled` alone cannot give it influence) + all 8 configs + session whitelists + `trailing_stop.strategy_overrides.wavelet_cycle` (1080 min static ceiling). **Dynamic per-trade time stop WIRED 2026-08-09** (was the last enablement prerequisite): the strategy publishes `metadata['time_stop_minutes']` (=1.5×cycle) and it is now honoured on both paths — live via `TrailingStopManager.register_time_stop(ticket, minutes)` (called from `TradingSystem._register_position_context` once MT5 returns the ticket) and in backtests via `SimulatedBroker._resolve_time_stop` (the two backtest engines copy the key onto `order.metadata`). ⚠️ **The config value is a CEILING, not the stop**: `effective = min(published, configured)`, so a strategy can only ever HOLD SHORTER than `trailing_stop.strategy_overrides.<name>.time_stop_minutes`. Registration is in-memory — after a restart an open position falls back to the configured ceiling (safe direction). This is exit *plumbing* only; it changes no research verdict and `wavelet_cycle` stays `enabled: false` at weight 0.00. ⚠️ Macro-correlation proxy is inverted EURUSD (no DXY price series in repo) and is injected only in research — live it degrades to pass-through with `macro_correlation: NaN`. **Enable ONLY after a NEW hypothesis clears the frozen OOS test WITH a surrogate margin AND 200 paper trades (spec §8.3).** Do NOT re-run the same search with a wider grid until a cell passes — that is the `project_rsi_reversal_m1` failure mode. Report: `reports/wavelet_cycle_research.md`. **Always backtest `--timeframe 15m`.** ⛔ **PREMISE FALSIFIED 2026-08-09 — do not revive this strategy** (`scripts/research_wavelet_scale.py`, `reports/wavelet_scale_invariance.md`, design `docs/superpowers/specs/2026-08-09-wavelet-scale-invariance-design.md`). A pre-registered scale-invariance test with a **passing positive control** settles what the earlier walk-forward could not: an injected 30-bar cycle is recovered at **30.1 bars in 3/3 MESA windows with prominence 21.8–76.0**, while real gold scores prominence **1.27/1.84/1.65 at windows 96/192/384 — below a pure random walk's 1.30/1.87/1.69 at every window**, with identical median periods (46.5/44.4/53.8) and identical IQRs. Gold's best prominence margin over any null is **+0.18** against a gate of 4.0. The "44-bar dominant cycle" is a property of the ESTIMATOR, not of gold. Because the detector demonstrably works, this is a statement about the market, not the method — so the three known measurement defects (see below) are NOT worth repairing. ⚠️ **Three defects found and left unrepaired, deliberately:** (1) `entry_dev_atr` is read only in the `MEAN_REVERT` branch, which produced **0 signals** — the published 12-cell grid was 6 cells run twice and the mean-reversion half was never tested. **Root-caused 2026-08-09 (`scripts/research_wavelet_meanrevert_funnel.py`): NOT a code bug — the branch is coherent and its conditions fire on 6.7% of tradeable bars when applied without the regime gate. It is STARVED:** MEAN_REVERT regime 2.9% of bars × cycle-tradeable 1.8% ⇒ they coincide on **0.04%** (6 of 15,598), one of which reached an entry before the MTF gate removed it. ⛔ **Do not loosen the regime gate to unblock it** — phase position and deviation sign are **independent on real gold (P(dev<0 | at_trough) = 0.47 vs P(dev>0) = 0.53)**, so the unlocked entries carry no information; this is the scale-invariance null arriving by a second route. Two bug hypotheses were tested and REFUTED: the phase/deviation convention is *not* inverted, and `entry_dev_atr` is *not* a unit mismatch (|dev|/ATR median 0.27, 21.9% of bars clear the 0.5 threshold). ⚠️ `TestMeanRevertEntries` passes on hand-built states and cannot see this reachability gap — green there is not evidence the path runs; (2) the harness collapses per-trade ATR stops (real spread 0.90–16.77pt, **18.6×**) to one median, so §7.2's vol-scaled stop was never exercised; (3) signals fire on **0.14% of bars** — the validate gate ran on 14 signals. ⚠️ **Latent trap in `src/cycles/spectral.py` regardless of this strategy:** `dominant_cycle` switches MESA→FFT when `x.size >= mesa_threshold` (128), and the FFT arm missed the injected sine at windows 96 and 192 (prominence 0.63/2.47 < 4.0) — **any future cycle work with `window >= 128` is silently blind**.

Support modules alongside the strategies:
- `base_strategy.py` — abstract base class
- `strategy_manager.py` — per-symbol, per-strategy instance registry + aggregation
- `strategy_allowlist.py` — default-deny filter (`SOLO_ALLOWED` / `KILL_LIST`), applied to every raw signal on both the live and backtest paths
- (`regime_filter.py` and `multi_timeframe_filter.py` were deleted 2026-08-13 — `regime_filter` was imported only by the deleted `vwap_strategy`, `multi_timeframe_filter` had no importers at all)
...
**2b. The allowlist — a new strategy CANNOT trade until it is listed**
- `src/strategies/strategy_allowlist.py` — add the canonical `strategy_name` to `SOLO_ALLOWED`. This is default-deny: a strategy that is registered, configured, weighted and enabled but absent from `SOLO_ALLOWED` is silently dropped before the risk engine ever sees it, on both the live and backtest paths. When deleting a strategy, move its name into `KILL_LIST` instead, so a stale config can't resurrect it.
...
`strategy_manager.py` import/register → **`strategy_allowlist.SOLO_ALLOWED`** → `STRATEGY_WEIGHTS` (3 regimes) → each `config_live_*.yaml` (`strategies.<name>` block + session whitelists) → `run_backtest.py` (`STRATEGY_CHOICES` + `create_strategy`) → `test_weights_table_completeness` `required_core` → unit test file → this CLAUDE.md table.

Skipping `SOLO_ALLOWED` is the failure mode with no error message: everything wires up, tests pass, and the strategy never trades.

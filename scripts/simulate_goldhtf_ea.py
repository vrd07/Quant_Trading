...
def get_htf_trend(h4, j, st):
    ...
    rsi_bull = (not INP["UseRSIFilter"]) or (40 < r < 80)
    rsi_bear = (not INP["UseRSIFilter"]) or (20 < r < 60)
...
def is_zone_mitigated(h1, j, typ, zone):
    """MQL5 index 0 = the FORMING H1 bar."""
...
def check_entry_confirmation(o, h, l, c, j, trend, htf_signal):
    ...
    def O(m): return o[j - m]
    def H(m): return h[j - m]
    def L(m): return l[j - m]
    def C(m): return c[j - m]
...
def run(m5, start, end):
    ...
    for j in range(60, len(m5)):
        ...
        if INP["UseDoubleFVG"] and k != cur_k:
            cur_k = k
            armed = None
            min_gap = h4_atr[k] * INP["DFVG_MinATR"]
...
def main():
    ...
    if ARGS.dfvg_only:
        INP["DFVG_Only"] = True
    for flag, key in (("lock_pct", "LockPct"), ("lock_trigger", "LockTriggerPct"),
    ...
    for kk in ["bars", "blocked_position_open", "blocked_session",
               ...
               "sltp_wrong_side", "lot_zero", "ENTRY_DFVG", "ENTRY_HTF"]:
